...
def convolve(image)
  ...
  Image.new(row_count: image.row_count+pad*2, column_count: image.column_count+pad*2).tap do |result|
    result.default_outside_grid = CONVOLUTION_TABLE.fetch(
      ([result.default_outside_grid]*9).map { CHAR_TO_DIGIT.fetch(_1) }.join.to_i(2)
    )
...
#INITIAL_IMAGE = Grid.from_rows(<<~GRID.lines.map { parse(_1) })
#  #..#.
#  #....
#  ##..#
#  ..#..
...
# puts convolve(convolve(INITIAL_IMAGE)).rows.map(&:join).join("\n")
puts convolve(convolve(INITIAL_IMAGE)).then { count_ones(_1) }
